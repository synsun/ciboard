...
class Cmalllib extends CI_Controller
{
    ...
    {
        $use = $this->CI->cmallconfig->item('use_cmall');
        return $use;
    }
    ...
    public function review_alarm($cre_id = 0)
    {
        ...
        $superadminlist = '';
        if ($this->CI->cmallconfig->item('cmall_email_admin_write_product_review')
            OR $this->CI->cmallconfig->item('cmall_note_admin_write_product_review')
            OR $this->CI->cmallconfig->item('cmall_sms_admin_write_product_review')) {

            $mselect = 'mem_id, mem_email, mem_nickname, mem_phone';
            $superadminlist = $this->CI->Member_model->get_superadmin_list($mselect);

        }
        if ($this->CI->cmallconfig->item('cmall_email_admin_write_product_review') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $emailsendlistadmin[$value['mem_id']] = $value;
            }
        }
        if (($this->CI->cmallconfig->item('cmall_email_user_write_product_review') && element('mem_receive_email', $member))
            OR $this->CI->cmallconfig->item('cmall_email_alluser_write_product_review')) {
            $emailsendlistuser['mem_email'] = element('mem_email', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_note_admin_write_product_review') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $notesendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_note_user_write_product_review') && element('mem_use_note', $member)) {
            $notesendlistuser['mem_id'] = element('mem_id', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_sms_admin_write_product_review') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $smssendlistadmin[$value['mem_id']] = $value;
            }
        }
        if (($this->CI->cmallconfig->item('cmall_sms_user_write_product_review') && element('mem_receive_sms', $member))
            OR $this->CI->cmallconfig->item('cmall_sms_alluser_write_product_review')) {
        ...
        $thumb_width = $this->CI->cmallconfig->item('cmall_product_review_thumb_width');
        $autolink = $this->CI->cmallconfig->item('use_cmall_product_review_auto_url');
        $popup = $this->CI->cmallconfig->item('cmall_product_review_content_target_blank');
        ...
        if ($emailsendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_admin_write_product_review_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_email_admin_write_product_review_content')
            );
        ...
        if ($emailsendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_user_write_product_review_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_email_user_write_product_review_content')
            );
            $this->CI->email->clear(true);
            $this->CI->email->from($this->CI->cbconfig->item('webmaster_email'), $this->CI->cbconfig->item('webmaster_name'));
        ...
        if ($notesendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_admin_write_product_review_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_admin_write_product_review_content')
            );
        ...
        if ($notesendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_user_write_product_review_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_user_write_product_review_content')
            );
            $note_result = $this->CI->notelib->send_note(
                $sender = 0,
        ...
        if ($smssendlistadmin) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_admin_write_product_review_content')
            );
            $sender = array(
        ...
        if ($smssendlistuser) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_user_write_product_review_content')
            );
            $sender = array(
                'phone' => $this->CI->cbconfig->item('sms_admin_phone'),
            );
    ...
    public function qna_alarm($cqa_id = 0)
    {
        ...
        $superadminlist = '';
        if ($this->CI->cmallconfig->item('cmall_email_admin_write_product_qna')
            OR $this->CI->cmallconfig->item('cmall_note_admin_write_product_qna')
            OR $this->CI->cmallconfig->item('cmall_sms_admin_write_product_qna')) {

            $mselect = 'mem_id, mem_email, mem_nickname, mem_phone';
            $superadminlist = $this->CI->Member_model->get_superadmin_list($mselect);

        }
        if ($this->CI->cmallconfig->item('cmall_email_admin_write_product_qna') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $emailsendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_email_user_write_product_qna') && $member && element('cqa_receive_email', $qna)) {
            $emailsendlistuser['mem_email'] = element('mem_email', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_note_admin_write_product_qna') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $notesendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_note_user_write_product_qna') && element('mem_use_note', $member)) {
            $notesendlistuser['mem_id'] = element('mem_id', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_sms_admin_write_product_qna') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $smssendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_sms_user_write_product_qna') && $member && element('cqa_receive_sms', $qna)) {
            if (element('mem_phone', $member)) {
                $smssendlistuser = $member;
            }
        }
        ...
        $receive_note = element('mem_use_note', $member) ? '동의' : '거부';
        $receive_sms = element('mem_receive_sms', $member) ? '동의' : '거부';
        $thumb_width = $this->CI->cmallconfig->item('cmall_product_qna_thumb_width');
        $autolink = $this->CI->cmallconfig->item('use_cmall_product_qna_auto_url');
        $popup = $this->CI->cmallconfig->item('cmall_product_qna_content_target_blank');
        ...
        if ($emailsendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_admin_write_product_qna_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_email_admin_write_product_qna_content')
            );
            foreach ($emailsendlistadmin as $akey => $aval) {
                $this->CI->email->clear(true);
                $this->CI->email->from($this->CI->cbconfig->item('webmaster_email'), $this->CI->cbconfig->item('webmaster_name'));
        ...
        if ($emailsendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_user_write_product_qna_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_email_user_write_product_qna_content')
            );
            $this->CI->email->clear(true);
            $this->CI->email->from($this->CI->cbconfig->item('webmaster_email'), $this->CI->cbconfig->item('webmaster_name'));
            $this->CI->email->to(element('mem_email', $emailsendlistuser));
        ...
        if ($notesendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_admin_write_product_qna_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_admin_write_product_qna_content')
            );
            foreach ($notesendlistadmin as $akey => $aval) {
                $note_result = $this->CI->notelib->send_note(
                    $sender = 0,
        ...
        if ($notesendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_user_write_product_qna_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_user_write_product_qna_content')
            );
            $note_result = $this->CI->notelib->send_note(
                $sender = 0,
                $receiver = element('mem_id', $notesendlistuser),
        ...
        if ($smssendlistadmin) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_admin_write_product_qna_content')
            );
            $sender = array(
                'phone' => $this->CI->cbconfig->item('sms_admin_phone'),
            );
        ...
        if ($smssendlistuser) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_user_write_product_qna_content')
            );
            $sender = array(
                'phone' => $this->CI->cbconfig->item('sms_admin_phone'),
            );
    ...
    public function qna_reply_alarm($cqa_id = 0)
    {
        ...
        $superadminlist = '';
        if ($this->CI->cmallconfig->item('cmall_email_admin_write_product_qna_reply')
            OR $this->CI->cmallconfig->item('cmall_note_admin_write_product_qna_reply')
            OR $this->CI->cmallconfig->item('cmall_sms_admin_write_product_qna_reply')) {

            $mselect = 'mem_id, mem_email, mem_nickname, mem_phone';
            $superadminlist = $this->CI->Member_model->get_superadmin_list($mselect);

        }
        if ($this->CI->cmallconfig->item('cmall_email_admin_write_product_qna_reply') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $emailsendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_email_user_write_product_qna_reply') && $member && element('cqa_receive_email', $qna)) {
            $emailsendlistuser['mem_email'] = element('mem_email', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_note_admin_write_product_qna_reply') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $notesendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_note_user_write_product_qna_reply') && element('mem_use_note', $member)) {
            $notesendlistuser['mem_id'] = element('mem_id', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_sms_admin_write_product_qna_reply') && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $smssendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_sms_user_write_product_qna_reply') && $member && element('cqa_receive_sms', $qna)) {
            if (element('mem_phone', $member)) {
                $smssendlistuser = $member;
            }
        }
        ...
        if ($emailsendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_admin_write_product_qna_reply_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_email_admin_write_product_qna_reply_content')
            );
            foreach ($emailsendlistadmin as $akey => $aval) {
                $this->CI->email->clear(true);
                $this->CI->email->from($this->CI->cbconfig->item('webmaster_email'), $this->CI->cbconfig->item('webmaster_name'));
        ...
        if ($emailsendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_user_write_product_qna_reply_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_email_user_write_product_qna_reply_content')
            );
            $this->CI->email->clear(true);
            $this->CI->email->from($this->CI->cbconfig->item('webmaster_email'), $this->CI->cbconfig->item('webmaster_name'));
            $this->CI->email->to(element('mem_email', $emailsendlistuser));
        ...
        if ($notesendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_admin_write_product_qna_reply_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_admin_write_product_qna_reply_content')
            );
            foreach ($notesendlistadmin as $akey => $aval) {
                $note_result = $this->CI->notelib->send_note(
                    $sender = 0,
        ...
        if ($notesendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_user_write_product_qna_reply_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_user_write_product_qna_reply_content')
            );
            $note_result = $this->CI->notelib->send_note(
                $sender = 0,
                $receiver = element('mem_id', $notesendlistuser),
        ...
        if ($smssendlistadmin) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_admin_write_product_qna_reply_content')
            );
            $sender = array(
                'phone' => $this->CI->cbconfig->item('sms_admin_phone'),
            );
        ...
        if ($smssendlistuser) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_user_write_product_qna_reply_content')
            );
            $sender = array(
                'phone' => $this->CI->cbconfig->item('sms_admin_phone'),
            );
    ...
    public function orderalarm($type = '', $cor_id = 0)
    {
        ...
        $superadminlist = '';
        if ($this->CI->cmallconfig->item('cmall_email_admin_' . $type)
            OR $this->CI->cmallconfig->item('cmall_note_admin_' . $type)
            OR $this->CI->cmallconfig->item('cmall_sms_admin_' . $type)) {

            $mselect = 'mem_id, mem_email, mem_nickname, mem_phone';
            $superadminlist = $this->CI->Member_model->get_superadmin_list($mselect);

        }
        if ($this->CI->cmallconfig->item('cmall_email_admin_' . $type) && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $emailsendlistadmin[$value['mem_id']] = $value;
            }
        }
        if (($this->CI->cmallconfig->item('cmall_email_user_' . $type) && element('mem_receive_email', $member)) OR $this->CI->cmallconfig->item('cmall_email_alluser_' . $type)) {
            $emailsendlistuser['mem_email'] = element('mem_email', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_note_admin_' . $type) && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $notesendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->cmallconfig->item('cmall_note_user_' . $type) && element('mem_use_note', $member)) {
            $notesendlistuser['mem_id'] = element('mem_id', $member);
        }
        if ($this->CI->cmallconfig->item('cmall_sms_admin_' . $type) && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $smssendlistadmin[$value['mem_id']] = $value;
            }
        }
        if (($this->CI->cmallconfig->item('cmall_sms_user_' . $type) && element('mem_receive_sms', $member))
            OR $this->CI->cmallconfig->item('cmall_sms_alluser_' . $type)) {
            if (element('mem_phone', $member)) {
                $smssendlistuser = $member;
            }
        }
        ...
        if ($emailsendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_admin_' . $type . '_title')
            );
        ...
        if ($emailsendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_email_user_' . $type . '_title')
            );
        ...
        if ($notesendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_admin_' . $type . '_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_admin_' . $type . '_content')
            );
            foreach ($notesendlistadmin as $akey => $aval) {
                $note_result = $this->CI->notelib->send_note(
                    $sender = 0,
        ...
        if ($notesendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_note_user_' . $type . '_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->cmallconfig->item('cmall_note_user_' . $type . '_content')
            );
            $note_result = $this->CI->notelib->send_note(
                $sender = 0,
                $receiver = element('mem_id', $notesendlistuser),
        ...
        if ($smssendlistadmin) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_admin_' . $type . '_content')
            );
            $sender = array(
                'phone' => $this->CI->cbconfig->item('sms_admin_phone'),
            );
        ...
        if ($smssendlistuser) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->cmallconfig->item('cmall_sms_user_' . $type . '_content')
            );
            $sender = array(
                'phone' => $this->CI->cbconfig->item('sms_admin_phone'),
             );
