 ...
 */
class Bannerclick extends CB_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'page/bannerclick';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Banner_click_log');
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_bannerclick_index';
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('bcl_id', 'ban_id', 'bng_name', 'banner.mem_id', 'banner_click_log.mem_id', 'bcl_datetime', 'bcl_ip'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('bcl_id', 'ban_id', 'banner.mem_id', 'banner_click_log.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_bannerclick_graph';
        $this->load->event($eventname);
        ...
        $result = $this->{$this->modelname}->get_banner_click_count($datetype, $start_date, $end_date, $orderby);
        $sum_count = 0;
        $arr = array();
        $max = 0;
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_bannerclick_listdelete';
        $this->load->event($eventname);
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_bannerclick_cleanlog';
        $this->load->event($eventname);
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->input->post('criterion') && $this->input->post('day')) {
                ...
                $this->Banner_click_log_model->delete_where($deletewhere);
                ...
                $log_count = $this->Banner_click_log_model->count_by($countwhere);
