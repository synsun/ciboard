...
/**
 * 관리자>컨텐츠몰관리>상품데모링크클릭 controller 입니다.
 */
class Linkclick extends CB_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'cmall/linkclick';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Cmall_demo_click_log', 'Cmall_item_meta');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Cmall_demo_click_log_model';
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_cmall_linkclick_index';
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('cdc_id', 'cit_id', 'cit_key', 'cmall_item.mem_id', 'cit_name', 'cmall_demo_click_log.mem_id', 'cdc_datetime', 'cdc_ip', 'cit_demo_admin_link', 'cit_demo_user_link'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('cdc_id', 'cit_id', 'cmall_item.mem_id', 'cmall_demo_click_log.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('cdc_id', 'cit_name', 'cit_key', 'cit_id'); // 정렬이 가능한 필드
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['item_member'] = $itemmember
                    = $this->Member_model->get_by_memid(element('item_mem_id', $val), $select);
                $result['list'][$key]['item_display_name'] = display_username(
                    element('mem_userid', $itemmember),
                    element('mem_nickname', $itemmember),
                    element('mem_icon', $itemmember)
                );
                $result['list'][$key]['itemurl'] = cmall_item_url(element('cit_key', $val));
                if (element('cdc_useragent', $val)) {
                    $userAgent = get_useragent_info(element('cdc_useragent', $val));
         ...
         */
        $search_option = array('cit_name' => '상품명', 'cit_key' => '상품코드', 'cit_demo_admin_link' => '관리자데모주소', 'cit_demo_user_link' => '사용자데모주소', 'cdc_datetime' => '날짜', 'cdc_ip' => 'IP');
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_cmall_linkclick_graph';
        $this->load->event($eventname);
        ...
        $result = $this->{$this->modelname}->get_link_click_count($datetype, $start_date, $end_date, $orderby);
        $sum_count = 0;
        $arr = array();
        $max = 0;
        ...
        if ($export === 'excel') {
            ...
            header('Content-Disposition: attachment; filename=상품데모링크클릭_' . cdate('Y_m_d') . '.xls');
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_cmall_linkclick_listdelete';
        $this->load->event($eventname);
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_cmall_linkclick_cleanlog';
        $this->load->event($eventname);
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->input->post('criterion') && $this->input->post('day')) {
                $deletewhere = array(
                    'cdc_datetime <=' => $this->input->post('criterion'),
                );
                $this->Cmall_demo_click_log_model->delete_where($deletewhere);
                ...
                $countwhere = array(
                    'cdc_datetime <=' => $criterion,
                );
                $log_count = $this->Cmall_demo_click_log_model->count_by($countwhere);
