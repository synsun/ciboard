...
/**
 * 관리자>컨텐츠몰관리>장바구니현황 controller 입니다.
 */
class Cmallcart extends CB_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'cmall/cmallcart';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Cmall_cart');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Cmall_cart_model';
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_cmall_cmallcart_index';
        ...
        $view['view']['sort'] = array(
            'cct_id' => $param->sort('cct_id', 'asc'),
            'cit_id' => $param->sort('cit_id', 'asc'),
            'cit_name' => $param->sort('cit_name', 'asc'),
            'cit_key' => $param->sort('cit_key', 'asc'),
            'cct_datetime' => $param->sort('cct_datetime', 'asc'),
            'cct_ip' => $param->sort('cct_ip', 'asc'),
        );
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('cct_id', 'cit_id', 'cct_datetime', 'cct_ip', 'cmall_cart.mem_id', 'cit_name', 'cit_key'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('cct_id', 'cmall_cart.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('cct_id', 'cit_id', 'cct_datetime', 'cct_ip', 'cit_name', 'cit_key'); // 정렬이 가능한 필드
         ...
         */
        $search_option = array('cit_name' => '상품명', 'cit_key' => '상품코드', 'cct_datetime' => '날짜', 'cct_ip' => '아이피');
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_cmall_cmallcart_rank';
        $this->load->event($eventname);
        ...
        if ($export === 'excel') {
            ...
            header('Content-Disposition: attachment; filename=장바구니_' . cdate('Y_m_d') . '.xls');
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_cmall_cmallcart_listdelete';
        $this->load->event($eventname);
