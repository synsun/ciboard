<div class="box">
    <div class="box-header">
        <ul class="nav nav-tabs">
            <li role="presentation" class="active"><a href="<?php echo admin_url($this->pagedir); ?>">충전통계</a></li>
        ...
        <div class="box-table-header">
            <form class="form-inline" name="flist" action="<?php echo current_url(); ?>" method="get" >
                ...
                <div class="btn-group" role="group" aria-label="...">
                    ...
                    <button type="button" class="btn <?php echo ($this->input->get('dep_from_type') === 'cash') ? 'btn-success' : 'btn-default'; ?> btn-sm" onclick="fcharge_submit('cash');">현금결제</button>
                    <?php if ($this->input->get('dep_from_type') === 'cash') { ?>
                        <button type="button" class="btn <?php echo ($this->input->get('method') === 'bank') ? 'btn-info' : 'btn-default'; ?> btn-sm" onclick="fmethod_submit('bank');">무통장</button>
                        <button type="button" class="btn <?php echo ($this->input->get('method') === 'card') ? 'btn-info' : 'btn-default'; ?> btn-sm" onclick="fmethod_submit('card');">카드</button>
                        <button type="button" class="btn <?php echo ($this->input->get('method') === 'realtime') ? 'btn-info' : 'btn-default'; ?> btn-sm" onclick="fmethod_submit('realtime');">실시간</button>
                        <button type="button" class="btn <?php echo ($this->input->get('method') === 'vbank') ? 'btn-info' : 'btn-default'; ?> btn-sm" onclick="fmethod_submit('vbank');">가상계좌</button>
                        <button type="button" class="btn <?php echo ($this->input->get('method') === 'phone') ? 'btn-info' : 'btn-default'; ?> btn-sm" onclick="fmethod_submit('phone');">핸드폰</button>
                        <button type="button" class="btn <?php echo ($this->input->get('method') === 'service') ? 'btn-info' : 'btn-default'; ?> btn-sm" onclick="fmethod_submit('service');">서비스</button>
                    <?php } ?>
                    <button type="button" class="btn <?php echo ($this->input->get('dep_from_type') === 'point') ? 'btn-success' : 'btn-default'; ?> btn-sm" onclick="fcharge_submit('point');">포인트결제</button>
                </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped table-bordered">
                ...
                <thead>
                    <tr>
                        <th>날짜</th>
                        <th><?php echo $this->depositconfig->item('deposit_name'); ?></th>
                        <th>현금</th>
                        <th>포인트</th>
                        <th>비율</th>
                        <th>그래프</th>
