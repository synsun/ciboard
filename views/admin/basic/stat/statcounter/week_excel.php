...
<table width="100%" border="1" bordercolor="#E4E4E4" cellspacing="0" cellpadding="0">
        ...
        if (element('list', $view)) {
            foreach (element('list', $view) as $key => $result) {
                if ($this->input->get('withoutzero') && ! element('scd_count', $result)) {
                    continue;
                }
        ?>
        <tr>
            <td height="40"><?php echo element($key, element('week_korean', $view)); ?></td>
            <td><?php echo element('scd_count', $result, 0); ?></td>
