...
?>
    <li class="table-list-item">
        <a href="<?php echo cmall_item_url(element('cit_key', $item)); ?>" class="thumbnail" title="<?php echo html_escape(element('cit_name', $item)); ?>">
            <img src="<?php echo thumb_url('cmallitem', element('cit_file_1', $item), 180, 180); ?>" alt="<?php echo html_escape(element('cit_name', $item)); ?>" title="<?php echo html_escape(element('cit_name', $item)); ?>" style="width: 100%; display: block;" />
        </a>
        ...
        <ul class="cmall-detail">
            ...
            <li class="cmall-price pull-right"> <?php echo number_format(element('cit_price', $item)); ?>원</li>
        </ul>
    </li>
<?php
        if ($k % 2 === 1 && $open) {
...
<div class="searchbox">
    <form class="navbar-form navbar-right pull-right" action="<?php echo current_url(); ?>" onSubmit="return itemSearch(this);">
        ...
        <div class="form-group">
            ...
            <input type="text" class="input px100" placeholder="Search" name="skeyword" value="<?php echo $this->input->get('skeyword'); ?>" />
