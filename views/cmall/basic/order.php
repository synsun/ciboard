...
<div class="well well-sm mb10 mt10">
    결제해야할 금액
...
?>
    ...
    <p class="market-title">구매하시는 분</p>
    <table class="table">
        <tr>
            <th>실명</th>
            <td><input type="text" name="mem_realname" class="input" value="<?php echo $this->member->item('mem_nickname'); ?>" /></td>
        </tr>
        <tr>
            <th>이메일</th>
        ...
        <tr>
            <th>휴대폰</th>
            <td><input type="text" name="mem_phone" class="input" value="<?php echo $this->member->item('mem_phone'); ?>" /></td>
        </tr>
        <tr>
            <th>하고싶은 말</th>
            <td> <textarea name="cor_content" class="input per90" cols="5"></textarea></td>
        </tr>
    </table>
    ...
    <table class="table">
        <tr>
            <th>총 주문금액</th>
            <td><strong><?php echo number_format($total_price_sum); ?>원</strong>
            <?php
            if ($this->depositconfig->item('use_deposit')) {
            ?>
                <br /><br />
                보유
                <?php echo html_escape($this->depositconfig->item('deposit_name')); ?> : (<?php echo number_format($this->member->item('total_deposit') + 0);?>
                <?php echo html_escape($this->depositconfig->item('deposit_unit')); ?>)중
                최대
                <?php
