    ...
    <table class="table table-striped table-bordered table-hover">
        <tbody>
            <tr>
                ...
                <td><?php echo number_format(element('cor_total_money', element('data', $view)) + 0);?> 원</td>
            </tr>
            <?php if (element('cor_deposit', element('data', $view))) { ?>
                <tr>
                    <td class="table-left"><?php echo html_escape($this->depositconfig->item('deposit_name')); ?>사용</td>
                    <td><?php echo number_format(element('cor_deposit', element('data', $view)) + 0);?> 원</td>
                </tr>
        <?php }
        $notyet = element('cor_cash_request', element('data', $view)) - element('cor_cash', element('data', $view)); ?>
            <tr class="info">
                <td class="table-left">결제액</td>
                <td><?php echo number_format(element('cor_cash', element('data', $view)) + 0);?> 원 <?php if ( ! $notyet) { echo '(완납)'; } ?></td>
            </tr>
            <?php if ($notyet) {?>
                <tr>
                    <td class="table-left">미결제액</td>
                    <td><?php echo number_format($notyet + 0);?> 원</td>
                </tr>
            <?php } ?>
        </tbody>
    </table>
