    ...
    <table class="table table-hover">
        ...
        ?>
            <tr>
                ...
                <td class="text-center"><?php echo number_format(element($i, element('register', $levelupconfig)) + 0); ?>일</td>
                <td class="text-center"><?php echo number_format(element($i, element('point_required', $levelupconfig)) + 0); ?>점</td>
                <td class="text-center"><?php echo number_format(element($i, element('post_num', $levelupconfig)) + 0); ?>개</td>
                <td class="text-center"><?php echo number_format(element($i, element('comment_num', $levelupconfig)) + 0); ?>개</td>
                <td class="text-center"><?php echo number_format(element($i, element('point_use', $levelupconfig)) + 0); ?>점</td>
                <td class="text-center">
                    <?php if ((int) element('next_level', $view) === $i) {?>
                        <button type="submit" class="btn btn-default btn-xs">레벨업신청</button>
                    <?php } else { ?>
                        -
                    <?php } ?>
                </td>
