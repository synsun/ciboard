...
<input type="hidden" name="site_name" value="<?php echo $this->cbconfig->item('company_name'); ?>" />
<input type="hidden" name="def_site_cd"     value="<?php echo element('pg_kcp_mid', element('pg', $view)); ?>">
...
<input type="hidden" name="good_name" value="<?php echo element('good_name', $view); ?>" />
...
<input type="hidden" name="bask_cntx" value="<?php echo isset($view['data']) ? count($view['data']) : 1; ?>" />

<!-- 장바구니 상품 상세 정보 (자바 스크립트 샘플(create_goodInfo()) 참고) -->
<input type="hidden" name="good_info" value="" />   <?php // 해당 입력값은 에스크로 일때만 입력합니다. ?>
