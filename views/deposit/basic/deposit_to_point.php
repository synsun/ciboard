...
<div class="modal-header">
    <h4 class="modal-title"><?php echo $this->depositconfig->item('deposit_name'); ?>을(를) 포인트로 전환</h4>
</div>
<div class="modal-body">
    <?php
    echo validation_errors('<div class="alert alert-warning" role="alert">', '</div>');
    echo show_alert_message(element('message', $view), '<div class="alert alert-auto-close alert-dismissible alert-info"><button type="button" class="close" >&times;</button>', '</div>');
    $attributes = array('class' => 'form-horizontal', 'name' => 'fwrite', 'id' => 'fwrite');
    echo form_open(current_full_url(), $attributes);
    ?>
        <div class="credit">
            <div class="credit_method">
                <div><?php echo $this->depositconfig->item('deposit_name'); ?> <span class="textblue"><?php echo number_format($this->member->item('total_deposit') + 0); ?><?php echo $this->depositconfig->item('deposit_unit'); ?></span>중, <input type="number" class="input px100" id="deposit" name="deposit" value="0" onkeydown="if (event.keyCode ==13) return false;" onkeyup="num_deposit_to_point()" /><?php echo $this->depositconfig->item('deposit_unit'); ?> 전환</div>
            </div>
            <div class="credit_method">
            ...
            <p><i class="fa fa-dot-circle-o"></i> <?php echo $this->depositconfig->item('deposit_name'); ?>를 포인트로 전환하실 수 있습니다</p>
...
function num_deposit_to_point() {
    ...
    point = Math.floor(deposit * <?php echo $this->depositconfig->item('deposit_refund_point'); ?>);
    result = '' + point;
